...
def To_Dictionary_1(df, product_name):
    ...
    KBar_dic['product'] = np.repeat(product_name, len(KBar_dic['open']))  # 已修正 .size 錯誤
...
def Change_Cycle(Date, cycle_duration, KBar_dic, product_name):
    ...
    for i in range(len(KBar_dic['time'])):  # 修正 .size 錯誤
    ...
    KBar_dic['product'] = np.repeat(product_name, len(KBar_dic['time']))  # 修正 .size 錯誤
...
def ChartOrder_RSI(Kbar_df, TR):
    ...
    for date, low, high in zip(Kbar_df['time'], Kbar_df['low'], Kbar_df['high']):
        BuyOrderPoint_date.append(date if date in [i[2] for i in BTR] else np.nan)
        BuyOrderPoint_price.append(low * 0.999 if date in [i[2] for i in BTR] else np.nan)
        BuyCoverPoint_date.append(date if date in [i[4] for i in BTR] else np.nan)
        BuyCoverPoint_price.append(high * 1.001 if date in [i[4] for i in BTR] else np.nan)
        SellOrderPoint_date.append(date if date in [i[2] for i in STR] else np.nan)
        SellOrderPoint_price.append(high * 1.001 if date in [i[2] for i in STR] else np.nan)
        SellCoverPoint_date.append(date if date in [i[4] for i in STR] else np.nan)
        SellCoverPoint_price.append(low * 0.999 if date in [i[4] for i in STR] else np.nan)

    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=Kbar_df['time'], y=Kbar_df['RSI'], mode='lines', line=dict(color='purple'), name='RSI'), secondary_y=False)

    fig.add_trace(go.Scatter(x=BuyOrderPoint_date, y=BuyOrderPoint_price, mode='markers', marker=dict(color='red', symbol='triangle-up', size=10), name='作多進場'), secondary_y=False)
    fig.add_trace(go.Scatter(x=BuyCoverPoint_date, y=BuyCoverPoint_price, mode='markers', marker=dict(color='blue', symbol='triangle-down', size=10), name='作多出場'), secondary_y=False)
    fig.add_trace(go.Scatter(x=SellOrderPoint_date, y=SellOrderPoint_price, mode='markers', marker=dict(color='green', symbol='triangle-down', size=10), name='作空進場'), secondary_y=False)
    fig.add_trace(go.Scatter(x=SellCoverPoint_date, y=SellCoverPoint_price, mode='markers', marker=dict(color='black', symbol='triangle-up', size=10), name='作空出場'), secondary_y=False)

    fig.update_layout(title='RSI 策略圖', yaxis2_showgrid=True)
...
def ChartOrder_MACD(Kbar_df, TR):
    ...
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=Kbar_df['time'], y=Kbar_df['macd'], mode='lines', name='MACD'), secondary_y=False)
    fig.add_trace(go.Scatter(x=Kbar_df['time'], y=Kbar_df['signal'], mode='lines', name='Signal'), secondary_y=False)

    # 下單點位畫圖...
    # (略，同上RSI)

    fig.update_layout(title='MACD 策略圖', yaxis2_showgrid=True)
    st.plotly_chart(fig, use_container_width=True)
...
def ChartOrder_Bollinger(Kbar_df, TR):
    ...
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Scatter(x=Kbar_df['time'], y=Kbar_df['BB_upper'], mode='lines', name='上軌', line=dict(color='gray')))
    fig.add_trace(go.Scatter(x=Kbar_df['time'], y=Kbar_df['BB_middle'], mode='lines', name='中軌', line=dict(color='blue')))
    fig.add_trace(go.Scatter(x=Kbar_df['time'], y=Kbar_df['BB_lower'], mode='lines', name='下軌', line=dict(color='gray')))

    # 下單點位畫圖...
    # (略，同上RSI)

    fig.update_layout(title='布林通道 策略圖', yaxis2_showgrid=True)
    st.plotly_chart(fig, use_container_width=True)
...
choices_strategies = [
    ...
    '<進場>: RSI < 30 作多, RSI > 70 作空. <出場>: 結算平倉.',
    '<進場>: DIF上穿MACD線作多, 下穿作空. <出場>: 結算平倉.',
    '<進場>: 收盤價 < 下軌作多, > 上軌作空. <出場>: 結算平倉.'
]
...
if choice_strategy == choices_strategies[0]:
    ...
    with st.expander("<策略參數設定>: RSI參數與停損設定"):
        RSIPeriod = st.slider('RSI週期', 2, 50, 14)
        RSI_floor = st.slider('進場: RSI小於(作多)', 10, 50, 30)
        RSI_exit = st.slider('出場: RSI大於(平倉)', 30, 80, 50)
    ...
    for n in range(last_nan_index_RSI + 1, len(KBar_df['time']) - 1):
        if not np.isnan(KBar_df['RSI'][n]):
            if OrderRecord.GetOpenInterest() == 0:
                if KBar_df['RSI'][n] < RSI_floor:
                    OrderRecord.Order('Buy', KBar_df['product'][n+1], KBar_df['time'][n+1], KBar_df['open'][n+1], Order_Quantity)
                    OrderPrice = KBar_df['open'][n+1]
                    StopLossPoint = OrderPrice - MoveStopLoss
            elif OrderRecord.GetOpenInterest() > 0:
                if KBar_df['RSI'][n] > RSI_exit or KBar_df['close'][n] < StopLossPoint:
                    OrderRecord.Cover('Sell', KBar_df['product'][n+1], KBar_df['time'][n+1], KBar_df['open'][n+1], Order_Quantity)

    ChartOrder_RSI(KBar_df, OrderRecord.GetTradeRecord())
    
elif choice_strategy == choices_strategies[2]:
    with st.expander("<策略參數設定>: MACD參數與停損設定"):
        fast_period = st.slider('快速線', 2, 30, 12)
        slow_period = st.slider('慢速線', 2, 60, 26)
        signal_period = st.slider('訊號線', 2, 30, 9)
        MoveStopLoss = st.slider('停損點數', 0, 100, 30)
        Order_Quantity = st.slider('下單數量', 1, 100, 1)
    ...
    for n in range(last_nan_index_MACD + 1, len(KBar_df['time']) - 1):
        if OrderRecord.GetOpenInterest() == 0:
    ...
    ChartOrder_MACD(KBar_df, OrderRecord.GetTradeRecord())

elif choice_strategy == choices_strategies[3]:
    with st.expander("<策略參數設定>: 布林通道週期與停損設定"):
        period = st.slider('布林週期', 2, 50, 20)
        std_dev = st.slider('標準差倍數', 1, 5, 2)
        MoveStopLoss = st.slider('停損點數', 0, 100, 30)
        Order_Quantity = st.slider('下單數量', 1, 100, 1)

    KBar_df = Calculate_Bollinger_Bands(KBar_df, period, std_dev)
    last_nan_index_BB = KBar_df['SMA'][::-1].index[KBar_df['SMA'][::-1].apply(pd.isna)][0]
    OrderRecord = Record()

    for n in range(last_nan_index_BB + 1, len(KBar_df['time']) - 1):
        if OrderRecord.GetOpenInterest() == 0:
    ...
    ChartOrder_Bollinger(KBar_df, OrderRecord.GetTradeRecord())
##### 繪製K線圖加上MA以及下單點位
# @st.cache_data(ttl=3600, show_spinner="正在加載資料...")  ## Add the caching decorator
# def ChartOrder_MA(Kbar_df,TR):
#     # # 將K線轉為DataFrame
